...
+ (void)load_Manual {
    ...
    _drag.queue = dispatch_queue_create("com.kergli.mac-mouse-fix.helper.modified-drag", attr);
...
static void handleMouseInputWhileInitialized(int64_t deltaX, int64_t deltaY, CGEventRef event) {
    ...
    if (MAX(fabs(ofs.x), fabs(ofs.y)) > _drag.usageThreshold) {
        ...
        /// Edit:
        ///   Lot's of people complained about this in 3.0.0 Beta 6. See https://github.com/noah-kergli/mac-mouse-fix/issues?q=is%3Aissue+is%3Aopen+label%3A%223.0.0+Beta+6+Click+and+Drag+Direction%22
