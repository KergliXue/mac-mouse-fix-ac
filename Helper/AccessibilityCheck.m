...
+ (void)load {
    ...
    if (args.count > 0) {
        ...
        if ([args containsObject:@"forceUpdateAccessibilitySettings"]) {
            ...
            /// See
            /// - https://github.com/noah-kergli/mac-mouse-fix/issues/415
            /// - https://github.com/noah-kergli/mac-mouse-fix/issues/412
            /// - https://github.com/noah-kergli/mac-mouse-fix/discussions/101 (Accessibility Guide)
    ...
    /// - It seems the NDEBUG flag is necessary to disable asserts. We hadn't had the NDEBUG flag set in 3.0.2 which contributed to a crashing issue issue where an assert was false (See: https://github.com/noah-kergli/mac-mouse-fix/issues/988) I'm not sure when we removed the NDEBUG flag.
