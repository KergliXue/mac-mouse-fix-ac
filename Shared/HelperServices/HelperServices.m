...
///     Unfortunately there have been problems with the availability APIs. See https://github.com/noah-kergli/mac-mouse-fix/issues/241.
...
///     Upate 14.08.2022 Still crashes for the dude. Made another change: All mentions of macOS 11, 12, 13 have been replaced with 11.0, 12.0, 13.0. Because all the examples on the internet write it like that. Let's see if that helps. Edit: That fixed it! See https://github.com/noah-kergli/mac-mouse-fix/issues/241
...
/**
 ...
 Example output of the `launchctl list mouse.fix.helper` command
 
     {
         ...
         "MachServices" = {
             "com.kergli.mac-mouse-fix.helper" = mach-port-object;
         };
...
+ (NSString *)executablePathForLaunchdIdentifier:(NSString *)identifier {
    ...
    if (@available(macOS 13.0, *)) {
        ...
        /// Under 14.2.1 I saw that `launchctl print gui/501/com.kergli.mac-mouse-fix.helper` sometimes contains the executable path, but only under these circumstances:
        ...
        /// - Despite this handling of the 'strange helper' situation, there are numerous reports of people not being able to open the app. See https://github.com/noah-kergli/mac-mouse-fix/issues/648.
...
static NSError *makeWritable(NSString *itemPath) {
    /**
     ...
     - For some reason, many users have had troubles enabling Mac Mouse Fix recently. Many of these troubles turned out to be due to the LaunchAgents folder having it's permissions set to 'read only'. This function can be used to fix that.
        - See for example Issue [#54](https://github.com/noah-kergli/mac-mouse-fix/issues/54)
...
static void removePrefpaneLaunchdPlist(void) {
    ...
    /// The launchd plist file used to be at `~/Library/LaunchAgents/com.kergli.mousefix.helper.plist` when the app was still a prefpane. In the very early days it was at `mouse.fix.helper.plist`.
    ...
    /// Now, with the app version, it's moved to `~/Library/LaunchAgents/com.kergli.mac-mouse-fix.helper.plist`
    ...
    NSArray<NSString *> *legacyLaunchdPlistPaths = @[[libraryPath stringByAppendingPathComponent:@"LaunchAgents/com.kergli.mousefix.helper.plist"],
